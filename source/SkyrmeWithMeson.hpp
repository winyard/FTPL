...
namespace FTPL {
    ...
    void SkyrmeModel::addVectorMeson(int meson) {
       //adds vector mesons in lightest order based selected integer meson
        V.push_back();
        V[V.size()-1] = createField(V[V.size()-1],false);
        //most of the vector mesons are complex vectors, so type is
