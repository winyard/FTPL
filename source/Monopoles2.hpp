...
#ifndef FTPL_MONOPOLES2_HPP
#define FTPL_MONOPOLES2_HPP
...
namespace FTPL {
    ...
    class YangMillsModel : public BaseFieldTheory {
    public:
        ...
        //required functions
        CUDA_HOSTDEV YangMillsModel(const char * filepath, bool isDynamic = false);
        CUDA_HOSTDEV YangMillsModel(int width, int height, int depth, bool isDynamic = false);
        CUDA_HOSTDEV ~YangMillsModel(){};
    ...
    double YangMillsModel::levi(int i, int j, int k, int l){
        if((i==j)||(i==k)||(i==l)||(j==k)||(j==l)||(k==l)) return 0;
        else return ( (i-j)*(i-k)*(i-l)*(j-k)*(j-l)*(k-l)/12 );
    }

    inline void YangMillsModel::RK4calc(int pos){
        if(inBoundary(pos)) {
...
//tryadding .noalias to A could be faster
    ...
    void YangMillsModel::setParameters(double Fpi_in, double epi_in, double mpi_in){
        Fpi = Fpi_in;
        epi = epi_in;
        mpi = mpi_in;
    }

    YangMillsModel::YangMillsModel(int width, int height, int depth, bool isDynamic): BaseFieldTheory(3, {width,height,depth}, isDynamic) {
        //vector<int> sizein = {width, height};
        //BaseFieldTheory(2,sizein);
        f = createField(f, isDynamic, true);
        Eigen::Vector3d minimum(-0.05,-0.05,-0.05);
        Eigen::Vector3d maximum(0.05,0.05,0.05);
        f->min = minimum;
        f->max = maximum;
    ...
    YangMillsModel::YangMillsModel(const char * filename, bool isDynamic): BaseFieldTheory(3, {10,10,10}, isDynamic){
        ...
        A1 = createField(A1, isDynamic, true);
        A2 = createField(A2, isDynamic, true);
        A3 = createField(A3, isDynamic, true);
        Eigen::Vector3d minimum(-0.01,-0.01,-0.01);
        Eigen::Vector3d maximum(0.01,0.01,0.01);
        A1->min = minimum;
        A1->max = maximum;
    ...
    double YangMillsModel::calculateEnergy(int pos){
        Eigen::Vector4d fx = single_derivative(f, 0, pos);
        Eigen::Vector4d fy = single_derivative(f, 1, pos);
        Eigen::Vector4d fz = single_derivative(f, 2, pos);
        Eigen::Vector4d A1x = single_derivative(A1, 0, pos);
        Eigen::Vector4d A1y = single_derivative(A1, 1, pos);
        Eigen::Vector4d A1z = single_derivative(A1, 2, pos);
        Eigen::Vector4d A2x = single_derivative(A2, 0, pos);
        Eigen::Vector4d A2y = single_derivative(A2, 1, pos);
        Eigen::Vector4d A2z = single_derivative(A2, 2, pos);
        Eigen::Vector4d A3x = single_derivative(A3, 0, pos);
        Eigen::Vector4d A3y = single_derivative(A3, 1, pos);
        Eigen::Vector4d A3z = single_derivative(A3, 2, pos);

        double sum = fx.squaredNorm() + fy.squaredNorm() + fz.squaredNorm()



        return (1.0/(e*e))*(fx.squaredNorm() + fy.squaredNorm() + fz.squaredNorm()
            + (pow(A->data[pos][0],2) + pow(A->data[pos][1],2) + pow(A->data[pos][2],2))*fsq
            + 2.0*A->data[pos][0]*(f->data[pos]*fx)/


        ) + (1.0/(e*e))*( pow(Ay[2]-Az[1],2) + pow(Az[0]-Ax[2],2) +pow(Ax[1]-Ay[0],2)  )

        double sum = (phi1x.squaredNorm()) + pow(lambda1y,2)*(phi1y.squaredNorm())
                     + (pow(lambda1x,2)*pow(A->data[pos][0],2) + pow(lambda1y,2)*pow(A->data[pos][1],2))*phi1sq(pos)
                     + 2.0*pow(lambda1x,2)*A->data[pos][0]*(phi1->data[pos][1]*phi1x[0]-phi1->data[pos][0]*phi1x[1])
                     + 2.0*pow(lambda1y,2)*A->data[pos][1]*(phi1->data[pos][1]*phi1y[0]-phi1->data[pos][0]*phi1y[1]);

        sum +=       pow(lambda2x,2)*(phi2x.squaredNorm()) + pow(lambda2y,2)*(phi2y.squaredNorm())
                     + (pow(lambda2x,2)*pow(A->data[pos][0],2) + pow(lambda2y,2)*pow(A->data[pos][1],2))*phi2sq(pos)
                     + 2.0*pow(lambda2x,2)*A->data[pos][0]*(phi2->data[pos][1]*phi2x[0]-phi2->data[pos][0]*phi2x[1])
                     + 2.0*pow(lambda2y,2)*A->data[pos][1]*(phi2->data[pos][1]*phi2y[0]-phi2->data[pos][0]*phi2y[1]);
        sum += pow( Ax[1]-Ay[0] ,2);
