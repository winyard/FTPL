...
inline float toSingleValue<float3>(float3 value) {
    return (float)(0.33f*(value.f[0]+value.f[1]+value.f[2]));
};
	...
	void transform(const S * from, T * to, unsigned int length, unsigned int start = 0) {
		...
		case HOUNSEFIELD:
			...
			if(typeid(T) != typeid(short))
				throw FTPLException("Has to be short.");
			...
			if(typeid(S) == typeid(short)) {
				...
				throw FTPLException("Source has to be ushort or short");
			}
		...
		case AVERAGE:
		    ...
		    if(!isVectorType(from)) {
		        throw FTPLException("Cannot convert scalar image using average intensity transformation");
		    }
		...
		case NORMALIZED:
			if(typeid(T) != typeid(float)) {
				throw FTPLException("Target has to float");
			} else {
		...
		case DEFAULT:
			...
			break;
		default:
			throw FTPLException("Invalid transformation type");
			break;
		}
	}
	template <class S, class T>
