...
#ifndef FTPL_MONOPOLES_HPP
#define FTPL_MONOPOLES_HPP
...
//tryadding .noalias to A could be faster
    ...
    YangMillsModel::YangMillsModel(const char * filename, bool isDynamic): BaseFieldTheory(3, {10,10,10}, isDynamic){
        ...
        A = createField(A, isDynamic, true);
        Eigen::Vector4d minimum(-0.01,-0.01,-0.01);
        Eigen::Vector4d maximum(0.01,0.01,0.01);
        A->min = minimum;
        A->max = maximum;
    ...
    double YangMillsModel::calculateEnergy(int pos){
        ...
        Eigen::Vector4d fy = single_derivative(f, 1, pos);
        Eigen::Vector4d fz = single_derivative(f, 2, pos);
        Eigen::Vector4d Ax = single_derivative(A, 0, pos);
        Eigen::Vector4d Ay = single_derivative(A, 1, pos);
        Eigen::Vector4d Az = single_derivative(A, 2, pos);
