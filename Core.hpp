...
template <class T>
class FieldTheory3 : public Dataset<T> {
    public:
        //FieldTheory3(std::string filename, IntensityTransformation IT = IntensityTransformation(DEFAULT)); // for reading mhd files
        FieldTheory3(const char * filename, int width, int height, int depth); // for reading raw files
        FieldTheory3(int width, int height, int depth); //create empty
        FieldTheory3(int3 size); //create empty
        template <class U>
        FieldTheory3(FieldTheory3<U> * otherFieldTheory3, IntensityTransformation IT = IntensityTransformation(DEFAULT)); //create copy?
        T get(int x, int y, int z) const; // get value
        T get(int3 pos) const; // get value
        T get(int i) const; // dont know????
        T * get(Region r) const; // pull region from the field theory
        void set(int x, int y, int z, T value); //set a value for the field theory
        void set(int3 pos, T v); //set a value for the field theory
        void set(int i, T v); //set something at a single int???
        void set(Region region, T value); // fix a region (e.g. boundary values)
        int getDepth() const;
        int3 getSize() const;
        void save(const char * filepath);
        void saveSlice(int slice, slice_plane direction, const char * filepath, const char * imageType);
        template <class U>
        FieldTheory3<T> & operator=(const FieldTheory3<U> &otherFieldTheory3); //set equal to other field theory
        bool inBounds(int x, int y, int z) const; // is the field theory within the bounds given
        bool inBounds(int3 pos) const; // is the field theory within the bounds given
        bool inBounds(int i) const; // is the integer within the bounds given
        int getTotalSize() const;
		Visualization * display();
		Visualization * display(float level, float window);
		...
		FieldTheory3<T> * crop(Region r) const;
    private:
        int depth;
};
